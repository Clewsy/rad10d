...
struct encoder *init_encoder(uint8_t channel_a, uint8_t channel_b)
{
	...
	if (gpioSetPullUpDown(channel_a, PI_PUD_UP)) return(NULL);	//Enable pull-up resistor on channel a pin.
	if (gpioSetPullUpDown(channel_b, PI_PUD_UP)) return(NULL);	//Enable pull-up resistor on channel b pin.

	if (gpioSetISRFunc(channel_a, EITHER_EDGE, 100, &volume_ISR)) return(NULL);	//Enable hardware interrupt input on encoder channel a pin and define routine to run.
	if (gpioSetISRFunc(channel_b, EITHER_EDGE, 100, &volume_ISR)) return(NULL);	//Enable hardware interrupt input on encoder channel b pin and define routine to run.
...
bool init_hardware(void)
{
	...
	if (gpioSetMode(TOGGLE_PIN, PI_INPUT)) return(FALSE);				//Set as input the pin to which the toggle button is connected.
	if (gpioSetPullUpDown(TOGGLE_PIN, PI_PUD_UP)) return(FALSE);			//Enable internal pull-up resistor on the toggle pin.
	if (gpioSetISRFunc(TOGGLE_PIN, RISING_EDGE, 0, &toggle_ISR)) return(FALSE);	//Set up interrupt on the toggle button.
